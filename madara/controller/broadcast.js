...
module.exports.addToRoom = async function (io, socket, req) {

  try {
    // console.log("Add to room",req.token,req)
    ...
    // logger.info("addToRoom: " + JSON.stringify(req));
    // console.log("add to room", req, req.eventId);
    if (req.marketId) {
      socket.join(req.marketId);
      Room.findOne({
        roomId: req.marketId,
        type: 'market'
      }).then(async room => {
        if (!room) {
          ...
          roomEntry.roomId = req.marketId;
          roomEntry.username = user.username;
          roomEntry.userId = user._id;
        ...
        console.log(error)
        logger.error("Error in saving room entry" + error);
      })
    }
    if (req.eventId) {
      if (req.type) {
        if (req.type == 'score') {
          socket.join(req.eventId + '-score');
          Room.findOne({
            roomId: req.eventId,
            type: 'score'
          }).then(async room => {
...
module.exports.removeFromRoom = async function (io, socket, req) {
  try {
    // console.log("remove from room",req.token,req)
    // let decoded = jwt.verify(req.token, myEnv.parsed.SECRET);
    let { userId } = jwt.decode(req.token);
    let user = await User.findOne({ _id: userId, token: req.token });
    if (!user) return io.to(socket.id).emit('logout');
    if (user.token != req.token) return io.to(socket.id).emit('logout');
    // logger.info("removeFromRoom: " + JSON.stringify(req));

    if (req.marketId) {
      socket.leave(req.marketId);
      Room.remove({
        roomId: req.marketId,
        type: 'market',
    ...
    if (req.eventId) {
      if (req.type) {
        if (req.type == 'score') {
          socket.leave(req.eventId + '-score');
          Room.remove({
            roomId: req.eventId,
            type: 'score',
            username: user.username,
...
module.exports.addToVirtualRoom = async function (io, socket, req) {
  try {
...
module.exports.removeFromVirtualRoom = async function (io, socket, req) {
  try {
    // console.log(req.token)
    // let decoded = jwt.verify(req.token, myEnv.parsed.SECRET);
    let { userId } = jwt.decode(req.token);
...
module.exports.virtualMarketPulse = function (io) {
  if (!io) return;
  ...
  Room.distinct('roomId', {
    ...
    if (dbRoomIds.length > 0) {
      Market.findOne({
        ...
        //  console.log(market.marketBook.status);
        // console.log(market.eventName);

        if (err) logger.debug(err);
        if (!market) return;
        if (market.timers == null) return;
        //  console.log(market.timers);
        ...
        if (timer == 0 || timer < 0) {
          ...
          if (timer > -45) {
            ...
            dbRoomIds.forEach(function (roomId, index) {
              //  console.log('market1'+roomId);
              io.to(roomId).emit('virtual-pulse-' + roomId, market);
            });
          } else {
            ...
            //  console.log('market2'+market.eventId);
            dbRoomIds.forEach(function (roomId, index) {
              // console.log('market1'+roomId);
              io.to(roomId).emit('virtual-pulse-' + roomId, market);
            });
          ...
          if (market.marketBook.status == "OPEN" || market.marketBook.status == "SUSPENDED") {
            ...
            dbRoomIds.forEach(function (roomId, index) {
              // console.log('market virtual pulse'+roomId);
              io.to(roomId).emit('virtual-pulse-' + roomId, market);
            });
          }
...
function updateMarketSteam(io, TeamID, OpponentID, status) {
  ...
  Market.findOne({
    ...
    if (io) {
      let dbMarket;
      if (status == 0) {
        dbMarket = market.HomeVirtual;
      }
      else {
        dbMarket = market.AwayVirtual;
      ...
      if (status == 0) {
        ...
        var totalrun = parseInt(dbMarket[n1].Run) + parseInt(dbMarket[n2].Run) + parseInt(dbMarket[n3].Run) + parseInt(dbMarket[n4].Run) + parseInt(dbMarket[n5].Run) + parseInt(dbMarket[n6].Run);
...
function updateManualMarketSteam(io, TeamID, OpponentID, status) {
  if (!io) return;
  ...
  Market.findOne({
    ...
    if (io) {
      ...
      CricketVideo.find({
        ...
        if (parseInt(market.sessionResult) == parseInt(TeamID)) {
          if (parseInt(market.Team1run) > parseInt(totalrun)) {
            updateManualMarketSteam(io, TeamID, OpponentID, 1);
            return;
          } else {
            setTimeout(function () {
              callbackBoardCast(io, dbMarket[n1]._id, market, TeamID, OpponentID, market.scoreHomeVirtual, market.scoreAwayVirtual, status, 1);
...
module.exports.updateMarketVirtual = function (io) {
  ...
  if (!io) return;
  // console.log("updateMarketVirtual");
  //createVirtualMarket();
  Market.findOne({
    'eventTypeId': 'v9'
  }).limit(1).sort({
    ...
    if (!market) {
      createVirtualMarket();
      console.log("create Virtual Market")
      return;
    }
    if (market.timers == null) return;
        ...
        if (market.vedioTime == false) {
          ...
          setTimeout(function () {
            Market.findOne({
              ...
              $natural: -1
            }).exec(function (err, kmarket) {
              if (kmarket.sessionResult) {

                updateManualMarketSteam(io, opponentId, teamId, 1);
              } else {

                updateMarketSteam(io, opponentId, teamId, 1);
              }
            });
...
async function createVirtualMarket() {

  console.log('market create');

  await VideoTeam.find({
    ...
    if (newItems.length > 1) {
      ...
      runners.push(selection1);

      await CricketVideo.find({
        'TeamID': newItems[0].TeamID,
        'OpponentID': newItems[1].TeamID,
      }, {
        TeamID: 1,
        OpponentID: 1,
        URL: 1,
        ...
        newHomeArr.push(dbMarket[n1]);
        newHomeArr.push(dbMarket[n2]);
        newHomeArr.push(dbMarket[n3]);
        newHomeArr.push(dbMarket[n4]);
        newHomeArr.push(dbMarket[n5]);
        newHomeArr.push(dbMarket[n6]);

      });

      await CricketVideo.find({
        'TeamID': newItems[1].TeamID,
        'OpponentID': newItems[0].TeamID,
      }, {
        TeamID: 1,
        OpponentID: 1,
        URL: 1,
        ...
        newAwayArr.push(dbMarket[n1]);
        newAwayArr.push(dbMarket[n2]);
        newAwayArr.push(dbMarket[n3]);
        newAwayArr.push(dbMarket[n4]);
        newAwayArr.push(dbMarket[n5]);
        newAwayArr.push(dbMarket[n6]);

      });


      if (newHomeArr.length == 0) {
        createVirtualMarket();
      }


      if (newAwayArr.length == 0) {
        createVirtualMarket();
      }
      ...
      var m2 = {
        ...
        vedioTime: false,
        HomeVirtual: newHomeArr,
        AwayVirtual: newAwayArr,
        runners: runners,
        managers: [],
        usersPermission: [],
        managerStatus: {},
