...
module.exports.getPaymentMethod = async (req, res) => {
    try {
        ...
        PaymentMethod.find({ typeId: '642173009688f92510eb7bee', paymenttype: { $ne: "manual" } }).sort({ createdAt: -1 })
            .then(doc => {
...
module.exports.withdrawalPayment = async (req, res) => {
    try {
        ...
        // console.log(getWithdrawnPayment)
        if (getWithdrawnPayment) return res.send({ success: false, message: `Withdrawal is pending.` });

        if (parseInt(dbUser.balance - dbUser.bounsBalance) < parseInt(req.body.amount)) return res.send({ success: false, message: "You don't have sufficients funds." });
